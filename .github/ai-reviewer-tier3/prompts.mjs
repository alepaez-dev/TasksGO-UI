...
For every concern you intend to clear, all five must succeed, in order:
  ...
  3. ENFORCING CODE — the file:line that enforces it, quoted verbatim, from code you actually read. Naming a mechanism ("it is optional", "it defaults", "it is sanitized", "it is intentional") is NOT a citation. A TEST IS NEVER ENFORCING CODE, and a test is never an oracle: you cannot run one, and a test shipped by THIS diff only encodes the behaviour this diff already has — so "the test still passes", "that matches what the test expects" and "the e2e assertion would catch it" prove nothing about correctness. If your reasoning lands on what a test would do, you answered the wrong question: restate the USER-VISIBLE property, ask what the test does NOT assert (that gap is usually the concern), and cite the production line.
...
STATES — Do not validate stateful behaviour only at its steady states. When the diff introduces or changes a transition, animation, delayed unmount, loading state, optimistic update, closing state, or any other intermediate state, ENUMERATE those states and check each one. A correct final state does not clear a defect that exists while entering or leaving it, and the endpoints of an interval tell you nothing about its interior. Pay special attention where two values intentionally DIVERGE during the interval — closed but still mounted, submitted but not yet confirmed, stale but not yet replaced: that divergence IS the window. Establish what ENDS the interval, and whether anything can stop that from firing — a fallback, a disabled transition, an unmount, a rejected promise — because that is what sets its true width.
...
- Accessibility defects that break behavior, in BOTH directions: an element that is keyboard-unreachable when it should be reachable, AND an element that is still operable — focusable, activatable — while the thing it lives in is closed, collapsed, or hidden. Hiding is THREE INDEPENDENT CHANNELS — from SIGHT, from ASSISTIVE TECH, from INTERACTION — and closing one does not close the others: opacity, size, clipping and transforms hide from sight only; \`aria-hidden\` hides from assistive tech only; \`pointer-events: none\` blocks the pointer only; \`inert\` closes assistive tech and interaction but leaves the content VISIBLE, so on its own it hides nothing a user can see. Only \`display:none\`, \`visibility:hidden\` or unmounting close all three. So when a diff hides, collapses, or closes something, say WHICH channels it closed, and whether that matches what it announced — a surface that reports itself closed while its contents stay operable is a defect, and the window it is wrong for is the whole exit transition, not one frame.
...
- You are given the CI check results at the reviewed commit. A green check never clears a concern its tests do not assert — but when your predicted failure is a quantity a green check's test DIRECTLY computes, the world has already measured it and disagreed: that refutes YOUR PREMISE (usually a platform-semantics assumption — browser layout, engine behaviour), not the code. Re-derive the premise; a finding filed against a green measurement must say why the check passed anyway.
